 ...
 *     defaultIcon: 'ti ti-app-window',
 *     defaultIconColor: 'var(--color-secondary)',
 *     headerBackground: 'var(--color-primary)',
 *     primaryButtonColor: 'var(--color-primary)',
...
class ModalsEverywhere {
    /**
     * Detect shared UI theme tokens if they are available
    ...
    constructor(config = {}) {
        // Detect shared theme tokens
        const themeTokens = this.detectThemeTokens();
        
        // Use shared theme tokens as defaults, allow config overrides
        this.config = {
            defaultIcon: config.defaultIcon || 'ti ti-circle',
            defaultIconColor: config.defaultIconColor || (themeTokens?.secondary || '#666'),
            headerBackground: config.headerBackground || (themeTokens?.primary || '#2c3e50'),
            primaryButtonColor: config.primaryButtonColor || (themeTokens?.primary || '#3498db'),
            secondaryButtonColor: config.secondaryButtonColor || '#95a5a6',
            dangerButtonColor: config.dangerButtonColor || (themeTokens?.danger || '#e74c3c'),
            ...config
        };
        ...
        if (themeTokens) {
            console.log('[Modals Everywhere] Integrated with shared UI theme tokens');
        }
    }
