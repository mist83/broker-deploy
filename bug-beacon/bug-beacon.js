    ...
    function scanLayoutIssues(reason) {
        ...
        for (const el of all) {
            ...
            if (cs.textOverflow === 'ellipsis' && cs.whiteSpace === 'nowrap'
                ...
                found.push({ kind: 'ellipsis-clip', sel: selectorForLayoutWatcher(el), text: text.slice(0, 80) });
            }
            ...
            if (cs.webkitLineClamp && cs.webkitLineClamp !== 'none' && cs.webkitLineClamp !== 'auto'
                ...
                found.push({ kind: 'line-clamp', sel: selectorForLayoutWatcher(el), clamp: cs.webkitLineClamp, text: text.slice(0, 80) });
            }
            ...
            if ((cs.overflow === 'hidden' || cs.overflowX === 'hidden' || cs.overflowY === 'hidden')
                ...
                found.push({ kind: 'overflow-hidden-clip', sel: selectorForLayoutWatcher(el), text: text.slice(0, 80) });
            }
            ...
            if (r.right > viewportW + 2 && cs.position !== 'fixed') {
                found.push({ kind: 'overflow-x-viewport', sel: selectorForLayoutWatcher(el), right: Math.round(r.right), vp: viewportW });
            }

            // 5. Tiny visible text (< 11px in body text)
            const fontPx = parseFloat(cs.fontSize);
            if (fontPx && fontPx < 11 && text.length > 0 && el.children.length === 0) {
                found.push({ kind: 'tiny-text', sel: selectorForLayoutWatcher(el), font: fontPx, text: text.slice(0, 40) });
            }
            ...
            if ((cs.overflowY === 'auto' || cs.overflowY === 'scroll')
                ...
                found.push({ kind: 'hidden-scroll-content', sel: selectorForLayoutWatcher(el), hidden: el.scrollHeight - el.clientHeight, visible: el.clientHeight, ratio: (el.scrollHeight / el.clientHeight).toFixed(2) });
            }
        }
