...
Proof comes in two accepted forms — operator's call, use whichever fits the work:

1. A short hosted **usage video** (the quality bar below), or
2. **At least 6 hosted screenshots** that walk the key states/interactions — initial state, the primary action, at least one state change, and the result. Six or more hosted shots that tell the story are a full `visual evidence: yes`, NOT a fallback.

Do not make random videos for video's sake. A single screenshot or a bare live URL is still only a fallback (see below).
...
When a usage video is not the right artifact, the **6+ hosted screenshots** form above is a full proof — mark `visual evidence: yes` with no "fallback" caveat. Only when even that is impossible (no meaningful visual surface, or capture blocked by the runtime) fall back to the strongest hosted evidence that exists (a single screenshot, live URL, commit SHAs, test summary, exact resume instructions). Put that fallback into the feed milestone when reachable so it is readable later. That weaker fallback line may be **yes** only when it is hosted/durable and the reason makes clear that it is a fallback. Otherwise it is **no** and `safe to close` is also **no**.
