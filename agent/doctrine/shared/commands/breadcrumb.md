...
Before answering, finish bounded closure work if this is nearly a natural stopping point: verification, commit/push where appropriate, deploy/live check, proof recording/upload, manifest sync, and the per-repo proof breadcrumb (`.proof.json` or equivalent).
