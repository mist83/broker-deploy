...
This command is not just a status question. If the task is close to a natural stopping point, finish bounded closure work first: verification, commit/push where appropriate, deploy/live check, proof recording/upload, manifest sync, and the per-repo proof breadcrumb (`.proof.json` or equivalent).
